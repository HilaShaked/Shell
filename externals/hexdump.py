    ...
    with open(args.FILE, "rb") as f:
        ...
        while True:
            ...
            if args.two_bytes_decimal:  # -d
                data = one_data_line
                data_len = len(data)
                decimal_string = " ".join([f'{int(f"{data[i + 1]:02x}{data[i]:02x}", 16)}'.zfill(5) if i + 1 < data_len
                                           else f"{data[i]}".zfill(5)
            ...
            if args.two_bytes_octal:  # -o
                data = one_data_line
                data_len = len(data)
                oct_string = " ".join([f'{int(f"{data[i + 1]:02x}{data[i]:02x}", 16):06o}' if i + 1 < data_len
                                       else f'{data[i]:06o}'
                                       for i in range(0, data_len, 2)])
            ...
            if args.two_bytes_hex:  # -x
                data = one_data_line
                data_len = len(data)
                hex_string = "   ".join([f"{data[i + 1]:02x}{data[i]:02x}" if i + 1 < data_len else f"{data[i]:04x}"
                                         for i in range(0, data_len, 2)])
